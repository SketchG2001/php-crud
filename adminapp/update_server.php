...
if (isset($_POST["submit"])) {
    ...
    if ($result->num_rows > 0) {
        ...
        if (isset($_FILES['file']) && $_FILES['file']['size'] > 0) {
            ...
            if (in_array($fileactualExt, $allowed)) {
                ...
                if ($fileerror === 0) {
                    if ($filesize < 1000000) {
                        ...
                        $stmt->bind_param('ssssssssssss', $newfname, $newlname, $newemail, $newpassword, $newmobile, $newdob, $fileDestination, $newgender, $newrole, $newskills, $newage, $username);
            ...
            $stmt = $conn->prepare("UPDATE userDB SET firstname=?, lastname=?, email=?,password=?, mobile=?, dob=?, gender=?, role=?, skills=?, age=? WHERE username=?");
            $stmt->bind_param('sssssssssss', $newfname, $newlname, $newemail, $newpassword, $newmobile, $newdob, $newgender, $newrole, $newskills, $newage, $username);
            if ($stmt->execute()) {
